...
class ResidualRecurrentDecoder(nn.Module):
    ...
    def init_hidden(self, hidden):
        ...
        """
        is_tensor_wrapper = isinstance(hidden, torchgraph.TensorWrapper)
        if (is_tensor_wrapper and hidden.tensor is not None) or (not is_tensor_wrapper and hidden is not None):
            # per-layer chunks
            if is_tensor_wrapper:
                hidden_tensor = hidden.tensor.chunk(self.num_layers)
                # (h, c) chunks for LSTM layer
                hidden.tensor = tuple(i.chunk(2) for i in hidden_tensor)
            else:
            ...
            if is_tensor_wrapper:
                hidden.tensor = [None] * self.num_layers
            else:

...
class BahdanauAttention(nn.Module):
    ...
    def set_mask(self, context_len, context):
        ...
        if isinstance(context_len, torchgraph.TensorWrapper):
            context_len = context_len.tensor
    ...
    def forward(self, query, keys):
        ...
        if isinstance(query, torchgraph.TensorWrapper):
            query = query.tensor
        if isinstance(keys, torchgraph.TensorWrapper):
            keys = keys.tensor
